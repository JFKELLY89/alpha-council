...
class ShadowBook:
    ...
    def compute(self, decision_id: str,
        ...
        # No fill means the executed variant is flat.
        if executed is None:
            executed = VariantState(
                variant=ShadowVariant.EXECUTED, structure=gpt.structure,
                qty=0, entry_debit=gpt.entry_debit,
                entry_timestamp=gpt.entry_timestamp,
                last_mark=gpt.last_mark, final_mark=gpt.final_mark)

        gpt_ps = gpt.pnl_per_spread()
        claude_ps = claude.pnl_per_spread() if claude.qty else 0.0
        exec_ps = executed.pnl_per_spread() if executed.qty else 0.0
